...
Fungi.KBMCtrl = class {
	...
	setDownOverride(d) {
		this.onDownOverride = d;
		return this;
	}
	onMouseWheel(e) {
		if (!this._activeHandler.onMouseWheel) return;

		var delta = Math.max(-1, Math.min(1, (e.wheelDelta || -e.detail))); // map between -1 and 1
...
Fungi.KBMCtrl_Viewport = class {
	constructor(camera) {
		var w = Fungi.gl.fWidth;
		var h = Fungi.gl.fHeight;
		this.camera = camera;
	...
	onMouseMove(e, ctrl, x, y, dx, dy) {
		// when shift is held, pan around; otherwise rotate
		if (!e.shiftKey) {
			this.camera.euler.y += dx * this.yRotRate;
			this.camera.euler.x += dy * this.xRotRate;
		}
